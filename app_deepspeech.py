...
def app_sst(model_path: str, lm_path: str, lm_alpha: float, lm_beta: float, beam: int):
    ...
    print("app_sst enetered - 2")
    status_indicator = st.empty()

    if not webrtc_ctx.state.playing:
        print("app_sst enetered - 3")
        return

    status_indicator.write("Loading...")
    text_output = st.empty()
    stream = None
    print("app_sst enetered - 4")
    while True:
        print("app_sst enetered - 5")
        if webrtc_ctx.audio_receiver:
            print("app_sst enetered - 6")
            if stream is None:
                from deepspeech import Model
                print("app_sst enetered - 7")
            ...
            try:
                print("app_sst enetered - 8")
                audio_frames = webrtc_ctx.audio_receiver.get_frames(timeout=1)
            except queue.Empty:
                time.sleep(0.1)
                status_indicator.write("No frame arrived.")
                print("app_sst enetered - 9")
                continue

            status_indicator.write("Running. Say something!")
            print("app_sst enetered - 10")
            ...
            if len(sound_chunk) > 0:
                ...
                print("app_sst enetered - text_output: ", text_output)
        else:
            status_indicator.write("AudioReciver is not set. Abort.")
            print("app_sst enetered - 12")
            break
